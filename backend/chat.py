...
# Sauvegarde toute les réponses du bot
def save_bot_response(conversation_id, current_user, text, current_tokens=0, message_tokens=0):
    ...
    try:
        ...
        return None
# Importation du modele d'embedding
try:
...
# FONCTION PRINCIPALE : Permet de POST un prompt et d'avoir la réponse retourné par le bot (Stream response)
@router.post("/chat")
async def chat(request: Request):
    ...
    current_tokens = 0
    message_tokens = 0
    # Si la limite de token a été atteinte, on affiche un mess
    ...
    # Analyse syntaxique de l'historique pour permettre de détecter les questions portant sur l'historique
    ...
    enriched_context = ""
    # On enrichit le contexte pour essayer de travailler sur l'historique de conversation
    if conversation_id in conversation_history:
        actual_questions = []
        for msg in conversation_history[conversation_id]:
            if msg.startswith("Question : "):
    ...
    # Fonction pour générer le stream réponse
    async def generate_stream():
        try:
            ...
            completion_stream = hf_client.chat.completions.create(
                ...
                messages=messages,
                max_tokens=1024, # Pour avoir un document plus long
                temperature=0.7,
                stream=True
            )
            chunk_buffer = ""
